...
public class SqliteDatabase
{
	...
	public SqliteDatabase (string dbName)
	{
		...
		{
			pathDB = Application.dataPath + "/StreamingAssets/"+dbName;
		}
		else
		{
			pathDB = System.IO.Path.Combine (Application.persistentDataPath, dbName);// koko's
		}

//		Debug.Log("PATH:"+pathDB);
		...
		if (!System.IO.File.Exists (pathDB)) {
			...
			string sourcePathChange="";
		
			foreach(char c in sourcePath.ToCharArray())
			{
				if(c == '/')
				{
					sourcePathChange += "\\";
